...
def ask_user_node(state: AgentState):
    """Sets the final answer to be the clarification question or a generic request for reformulation, potentially including the low-confidence answer."""
    print("--- Node: Ask User for Clarification ---")
    
    specific_question = state.get("clarification_question")
    low_confidence_answer = state.get("final_answer") # Get the answer generated before this node
    
    if specific_question:
        # Came from validation: just ask the specific question
        final_message = specific_question
        print(f"Using specific clarification question: {final_message}")
    else:
        # Came from low confidence generation: show the answer and ask for reformulation
        generic_request = "Generé la siguiente respuesta, pero no estoy completamente seguro de su precisión o relevancia. ¿Podrías reformular tu pregunta o proporcionar más detalles para que pueda intentarlo de nuevo?"
        # Combine the original answer (if available) with the request
        if low_confidence_answer and isinstance(low_confidence_answer, str):
             final_message = f"{low_confidence_answer}\n\n---\n\n{generic_request}"
             print(f"Combining low-confidence answer with generic request.")
        else:
             # Fallback if the previous answer wasn't stored correctly
             final_message = generic_request
             print(f"Using generic low-confidence/reformulation request (previous answer not found): {final_message}")
             
    # In a real application, this node might signal the main loop to pause.
    # Here, we just set the final_answer to the message and end the graph run.
    return {"final_answer": final_message} # Update state and prepare to end
